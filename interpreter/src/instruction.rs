...
impl<T> Timed<T> {
    pub fn new(op: T, time: ILongWord) -> Self {
        Timed { op, time }
    }
}
...
/// The argument of an instruction that a pc offset
...
fn read_time(m: &mut Machine) -> ILongWord {
    let low = m.read_pc();
    let high = m.read_pc().cast_to_signed();    
    ILongWord { low, high }
}
...
fn write_time(m: &mut Machine, x: &ILongWord) {
    m.write_pc(x.low);
    m.write_pc(x.high.cast_to_unsigned());
}
...
impl Operand {
    fn decode(m: &mut Machine, mode: UWord) -> Self {
        ...
        match time_flag {
            true => match operand_flag {
                ...
                0x4 => Timed { op: Imm(read_word(m)),     time: 0.into()     },
                _ => unreachable!(),
            }
            false => match operand_flag {
                0x0 => Timed { op: Reg(read_register(m)), time: 0.into() },
                0x1 => Timed { op: Abs(read_address(m)),  time: 0.into() },
                0x2 => Timed { op: Ind(read_address(m)),  time: 0.into() },
                0x3 => Timed { op: Abx(read_address(m)),  time: 0.into() },
                0x4 => Timed { op: Imm(read_word(m)),     time: 0.into() },
                _ => unreachable!(),
            }
        }
    ...
    fn encode(m: &mut Machine, x: &Operand) -> () {
        use Op::*;
        let time_flag = x.time.value() != 0;
        let mode = |a,b| UWord::from((a as u8) | (b as u8));
        match &x.op {
            Reg(y) => {
...
impl Operands {
    fn decode(m: &mut Machine, mode: UWord) -> Self {
        ...
        let src = Operand::decode(m, UWord::from(src_mode | time_mode));
        let dst = Operand::decode(m, UWord::from(dst_mode | time_mode));
        Operands { src, dst }
    }
    ...
    fn encode(m: &mut Machine, x: &Operands) -> () {
        use Op::*;
        let mut mode = 0;
        let time_flag = x.src.time.value() != 0 || x.dst.time.value() != 0;
...
impl Instruction {
    ...
    pub fn encode(m: &mut Machine, instruction: &Instruction) {
        ...
        match instruction {
            ...
            Clc => {
                m.write_pc(UWord::from(0x38));
            }
            Sec => {
                m.write_pc(UWord::from(0x39));
            }
            Hcf => {
            ...
            Lsl(op) => {
                m.write_pc(UWord::from(0x1b));
                Operand::encode(m, op);
            }
            Lsr(op) => {
                m.write_pc(UWord::from(0x1c));
                Operand::encode(m, op);
            }
            Asr(op) => {
            ...
            Inc(op) => {
                m.write_pc(UWord::from(0x1e));
                Operand::encode(m, op);
            }
            Dec(op) => {
                m.write_pc(UWord::from(0x1f));
                Operand::encode(m, op);
            }
            Cmp(op) => {
            ...
            Jmp(x) => {
                m.write_pc(UWord::from(0x30));
                write_address(m, x);
            }
            Bcc(x) => {
                m.write_pc(UWord::from(0x31));
                write_offset(m, x);
            }
            Bcs(x) => {
                m.write_pc(UWord::from(0x32));
                write_offset(m, x);
            }
            Bne(x) => {
                m.write_pc(UWord::from(0x33));
                write_offset(m, x);
            }
            Beq(x) => {
                m.write_pc(UWord::from(0x34));
                write_offset(m, x);
            }
            Bpl(x) => {
                m.write_pc(UWord::from(0x35));
                write_offset(m, x);
            }
            Bmi(x) => {
                m.write_pc(UWord::from(0x36));
                write_offset(m, x);
            }
            Cal(x) => {
                m.write_pc(UWord::from(0x37));
                write_address(m, x);
            }
        }
    }
