...
pub const MAX_UNSIGNED_WORD_VALUE: u8 = 1 << WORD_SIZE;

pub mod sig {
    pub trait Signature: Into<u8> {}
...
where
    S: sig::Signature + num_traits::Zero
{
    fn default() -> Self {
        Self { value: S::zero(), phantom: Default::default() }
    }
}
...
impl From<i8> for Word<sig::Signed> {
    fn from(value: i8) -> Self {
        debug_assert!((value as u8) < MAX_UNSIGNED_WORD_VALUE);
        Word {
            value: (value as u8),
            phantom: PhantomData::default(),
        }
