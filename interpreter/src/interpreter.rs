...
fn operand_to_ref<'a>(state: &'a Machine, operand: &'a Operand, universe: &Universe) -> &'a UWord {
    use Register as R;
...
fn execute(state: &mut Machine, instruction: &Instruction, universe: &mut Universe) {
    ...
    match instruction {
        ...
        Instruction::Mov(Operands { src, dst }) => {
            let word = *mk_ref(state, &src, universe);
            *mk_mref(state, &dst, universe) = word;
            set_flag_nvz(state, &word);
        }
        Instruction::Psh(x) => {
            state.write_sp(*mk_ref(state, &x, universe));
        }
        Instruction::Pop(x) => {
            let word = state.read_sp();
            *mk_mref(state, &x, universe) = word;
            set_flag_nvz(state, &word);
        }

        // Arithmetic
        Instruction::Add(Operands { src, dst }) => {
            let src_orig = mk_ref(state, &src, universe).value();
            let dst_orig = mk_ref(state, &dst, universe).value();
            let result = src_orig + dst_orig + u8::from(state.cpu.flags.read(Flag::C));
        ...
        Instruction::Mul(Operands { src, dst }) => {
            let result =
                mk_ref(state, &dst, universe).value() * mk_ref(state, &src, universe).value();
        ...
        Instruction::Muh(Operands { src, dst }) => {
            // Multiply normally
            let result: u16 = mk_ref(state, &dst, universe).value() as u16
                * mk_ref(state, &src, universe).value() as u16;
        ...
        Instruction::Mus(Operands { src, dst }) => {
            ...
            let result = sign_extend(mk_ref(state, &dst, universe).value())
                * sign_extend(mk_ref(state, &src, universe).value());
            let word = UWord::from((result >> WORD_SIZE) as u8);
            *mk_mref(state, &dst, universe) = word;
            set_flag_nvz(state, &word);
        }
        ...
        Instruction::And(Operands { src, dst }) => {
            let result =
                mk_ref(state, &src, universe).value() & mk_ref(state, &dst, universe).value();
        ...
        Instruction::Not(x) => {
            let result = !mk_ref(state, &x, universe).value();
            let word = UWord::from(result & 0b00111111);
            *mk_mref(state, &x, universe) = word;
            set_flag_nvz(state, &word);
        }
        Instruction::Lsl(x) => {
            let result = mk_ref(state, &x, universe).value() << 1;
        ...
        // Jumping
        Instruction::Jmp(addr) => state.cpu.pc = *addr,
...
pub fn step(state: &mut Machine, universe: &mut Universe) {
