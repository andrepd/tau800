...
%\item[A]
%\end{itemize}
%Then, the CPU has one flag register \ttt{F}, with the following bits: \verb|N--PZC|, respectively negative, parity, zero, and carry flags. 
...
\instruction{BIT src dst}{Flags: set \ttt{Z} to $\neg(\texttt{src}\land\texttt{dst})$. Set \ttt{N} to msb of $\ttt{src}\land\ttt{dst}$.}
