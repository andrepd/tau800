...
\usepackage{tikz}
\usetikzlibrary{shapes,arrows,positioning,calc,decorations,arrows.meta,matrix}
\input{tikz}
...
\makeatletter
\def\@machinename@short{{{\sffamily TAU-800A}}}
\def\@machinename@long{{\textsf{Time Arithmetic Unit} (Revision 800A)}}
...
\noindent
Congratulations on becoming the proud owner of a \machinename*{}, the latest technology
...
\begin{center}
	\begin{tabular}{|l|l|l|}
		\hline
		\textbf{Mode} & \textbf{Notation} & \textbf{Description}                             \\ \hline
		Address       & \ttt{llhh}        & Absolute address                                 \\ \hline
		Offset        & \ttt{ff}          & Signed offset written as a two's complement word \\ \hline
	\end{tabular}
\end{center}

\subsubsection*{Temporal addressing}

Any addressing mode detailed in the first table may be suffixed with \ttt{@+dd} or \ttt{@-dd} for a temporal addressing of \ttt{dd}\footnote{A decimal number.} clock cycles into the future or into the past, respectively. 
The assembler will automatically compute the correct surge voltage to produce the desired time jump. 

\vspace{1em}
\warningbox{\null\hfill\textbf{Over-``Clocking''}\hfill\null\\
	\textbf{DO NOT} attempt to alter the operating clock frequency of the TAU computing
	unit by tampering with the clock generator in the causal-consistency bus. Damaging the
	warranty breakaway sticker on your TAU computing unit will \underline{immediately void your warranty}.
	\companyname{} cannot be held responsible for accidents resulting from operating
	the TAU computing unit at non-standard clocking speeds.}
...
\instruction{POP dst}{Pop value on the top of the stack onto \ttt{dst} and decrement \ttt{sp}. \\ Flags: set \ttt{Z}, \ttt{N}, \ttt{V}.}

\subsubsection*{Arithmetic}

\instruction{ADD src dst}{%
	Set word at \texttt{dst} to $\texttt{dst}+\texttt{src}+\ttt{C}$. \\
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}, set \ttt{C} on overflow.
}

\instruction{SUB src dst}{%
	Set word at \texttt{dst} to $\texttt{dst}-\texttt{src}-\overline{\ttt{C}}$. \\
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}, set \ttt{C} on underflow.
}

\instruction{MUL src dst}{%
	Set word at \texttt{dst} to the lower bits of $\texttt{dst}×\texttt{src}$. \\
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}. %, set \ttt{C} on overflow.
}

%TODO corrigir que tecnicamente o V é junk em tudo menos no cmp e sub

\instruction{MUH src dst}{%
	Set word at \texttt{dst} to the upper bits of $\texttt{dst}×\texttt{src}$, where operands are unsigned values. \\
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}.
}

\instruction{MUS src dst}{%
	Set word at \texttt{dst} to the upper bits of $\texttt{dst}×\texttt{src}$, where operands are signed values. \\
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}.
}

%\todo{Como fazer handle de divisoes por zero?}
\instruction{DIV src dst}{%
	Set word at \texttt{dst} to $(\texttt{dst}+\ttt{C}\times 2^6)\div\texttt{src}$, where operands are unsigned values. 
	Division by zero sets \ttt{dst} to an undefined value. \\
	Flags: set \ttt{Z}.
}
...
\instruction{MOD src dst}{%
	Set word at \texttt{dst} to $(\texttt{dst}+\ttt{C}\times 2^6)\mod\texttt{src}$, where operands are unsigned values. 
	Division by zero sets \ttt{dst} to an undefined value. \\
	Flags: set \ttt{Z}.
}
...
\instruction{LSR dst}{%
	Shift bits in \ttt{dst} to the right, plus $\ttt{C}\times 2^6$. \\
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}.
}

\instruction{INC dst}{%
	Increment \ttt{dst} by 1. 
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}.
}

\instruction{DEC dst}{%
	Decrement \ttt{dst} by 1. 
	Flags: set \ttt{Z}, \ttt{N}, \ttt{V}.
}
...
\instruction{CAL addr}{Store current \texttt{pc} in stack, increment \texttt{sp}, and jump to \texttt{addr}.}
...
\instruction{HCF}{Halt execution.}



\subsection*{Memory map}

Upon power-on, the system boots from memory location \ttt{\$80}. 
In order to write addresses \ttt{\$80}–\ttt{\$3f}, you have to reprogram the EPROM; please refer to technical procedure \S 8.2 in the User's Manual. 

The initial stack location is \ttt{\$7f}, and the stack grows downwards. It is the user's responsibility to avoid overflows and corruption of protected memory locations!

Memory locations \ttt{\$10}–\ttt{\$6f} control the unit's IO devices and peripherals. 
For more details, please refer to the manual of your specific device or peripheral. 

\smallskip
\warningbox{Use \underline{only} Approved Genuine Devices\textregistered from \companyname{}, its partners, and approved vendors. \companyname{} is not responsible for any damage sustained while an unapproved device or peripheral is connected to your unit.}

The memory-map is summarised in \autoref{fig:memmap}. 
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		...
		\draw (mem-1-1.south west) -- +(0,-0.5) node[below,font=\ttfamily] {\$00};
		\draw (mem-1-2.south west) -- +(0,-0.5) node[below,font=\ttfamily] {\$10};
		\draw (mem-1-3.south west) -- +(0,-0.5) node[below,font=\ttfamily] {\$70};
		\draw (mem-1-4.south west) -- +(0,-0.5) node[below,font=\ttfamily] {\$80};
		\draw (mem-1-5.south west) -- +(0,-0.5) node[below,font=\ttfamily] {\$3f};
		
		\draw[white,thick,dashed] (mem-1-5.north east) -- (mem-1-5.south east);
	\end{tikzpicture}
%	\caption{Memory map of \verb|\machinename|.}
%	\caption{Memory map of \machinename.}
	\caption{Memory map of .}
	\label{fig:memmap}
\end{figure}
...
\clearpage
\section{I/O Via the Card Tray}

The I/O is done via 80-column card, inserted in the Input Tray as shown in figure
\ref{fig:input_tray}. \machinename{} units are compatible with various formats of
Program Card, and may require a different setup for compatibility. Therefore, much
of this information may not be relevant for you, wherefore we refer to the complete
reference manual (``Program Card Input Tray'' section) for more information.
...
\begin{figure}[hbp]
	...
	\centerline{%
		\fbox{\parbox{\linewidth}{%
		    \hspace{5em}%
			\pdfimageresolution=200%
			\includegraphics{assets/feed.png}%
		}}
	}
	\caption{Program Cards are inserted in the Input Tray as shown.}
	\label{fig:input_tray}
\end{figure}
...
\centerline{\ttfamily Do not distribute. \copyright*{} 1971}
