...
\instruction{DIV src dst}{%
	Set word at \texttt{dst} to $\texttt{dst}\div\texttt{src}$, where operands are unsigned values. \\
	Flags: set \ttt{Z}.
}
...
\instruction{MOD src dst}{%
	Set word at \texttt{dst} to $\texttt{dst}\mod\texttt{src}$, where operands are unsigned values. \\
	Flags: set \ttt{Z}.
}
