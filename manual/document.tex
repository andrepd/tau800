...
\todo{O que acontece em SUB se C estiver set?}
...
\instruction{CMP src dst}{If $\texttt{src}>\texttt{dst}$ set carry flag. If $\texttt{src}=\texttt{dst}$ set zero flag.}

%\instruction{TST src dst}{If $\texttt{src}>\texttt{dst}$ set carry flag. If $\texttt{src}=\texttt{dst}$ set zero flag.}
...
\instruction{JMP dst}{Jump execution to \texttt{dst}.}
